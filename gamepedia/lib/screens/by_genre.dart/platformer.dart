...
class PlatformerScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final platformerGames = gameList
        .where(
          (game) => game.genre.any((g) => g.toLowerCase().contains('platform')),
        )
        .toList();
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: platformerGames.isEmpty
              ? const Center(
                  child: Text(
                    "No platformer games found.",
                    style: TextStyle(color: Colors.white70, fontSize: 16),
                  ),
                )
              : ListView.builder(
                  itemCount: platformerGames.length,
                  itemBuilder: (context, index) {
                    final game = platformerGames[index];
