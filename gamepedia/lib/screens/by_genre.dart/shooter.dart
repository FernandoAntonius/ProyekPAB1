...
class ShooterScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final shooterGames = gameList
        .where(
          (game) => game.genre.any((g) => g.toLowerCase().contains('shooter')),
        )
        .toList();
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: shooterGames.isEmpty
              ? const Center(
                  child: Text(
                    "No shooter games found.",
                    style: TextStyle(color: Colors.white70, fontSize: 16),
                  ),
                )
              : ListView.builder(
                  itemCount: shooterGames.length,
                  itemBuilder: (context, index) {
                    final game = shooterGames[index];
