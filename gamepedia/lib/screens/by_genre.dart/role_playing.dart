...
class RolePlayingScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final rolePlayingGames = gameList
        .where(
          (game) => game.genre.any((g) => g.toLowerCase().contains('role-playing')),
        )
        .toList();
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: rolePlayingGames.isEmpty
              ? const Center(
                  child: Text(
                    "No role-playing games found.",
                    style: TextStyle(color: Colors.white70, fontSize: 16),
                  ),
                )
              : ListView.builder(
                  itemCount: rolePlayingGames.length,
                  itemBuilder: (context, index) {
                    final game = rolePlayingGames[index];
