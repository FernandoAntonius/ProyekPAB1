...
class PuzzleScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final puzzleGames = gameList
        .where(
          (game) => game.genre.any((g) => g.toLowerCase().contains('puzzle')),
        )
        .toList();
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: puzzleGames.isEmpty
              ? const Center(
                  child: Text(
                    "No puzzle games found.",
                    style: TextStyle(color: Colors.white70, fontSize: 16),
                  ),
                )
              : ListView.builder(
                  itemCount: puzzleGames.length,
                  itemBuilder: (context, index) {
                    final game = puzzleGames[index];
