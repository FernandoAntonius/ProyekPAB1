...
class NintendoSwitchScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final nintendoSwitchDevice = gameList
        .where(
          (game) => game.device.any((g) => g.toLowerCase().contains('nintendo switch')),
        )
        .toList();
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: nintendoSwitchDevice.isEmpty
              ? const Center(
                  child: Text(
                    "No nintendo switch device found.",
                    style: TextStyle(color: Colors.white70, fontSize: 16),
                  ),
                )
              : ListView.builder(
                  itemCount: nintendoSwitchDevice.length,
                  itemBuilder: (context, index) {
                    final game = nintendoSwitchDevice[index];
