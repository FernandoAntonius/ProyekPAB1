...
class LinuxScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final linuxDevice = gameList
        .where(
          (game) => game.device.any((g) => g.toLowerCase().contains('linux')),
        )
        .toList();
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: linuxDevice.isEmpty
              ? const Center(
                  child: Text(
                    "No linux device found.",
                    style: TextStyle(color: Colors.white70, fontSize: 16),
                  ),
                )
              : ListView.builder(
                  itemCount: linuxDevice.length,
                  itemBuilder: (context, index) {
                    final game = linuxDevice[index];
