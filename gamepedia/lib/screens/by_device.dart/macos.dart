...
class MacOSSCreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final macosDevice = gameList
        .where(
          (game) => game.device.any((g) => g.toLowerCase().contains('mac')),
        )
        .toList();
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Padding(
          ...
          child: macosDevice.isEmpty
              ? const Center(
                  child: Text(
                    "No macos device found.",
                    style: TextStyle(color: Colors.white70, fontSize: 16),
                  ),
                )
              : ListView.builder(
                  itemCount: macosDevice.length,
                  itemBuilder: (context, index) {
                    final game = macosDevice[index];
